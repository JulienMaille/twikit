...
class Tweet:
    ...
    def __init__(self, client: Client, data: dict, user: User = None) -> None:
        ...
        self.text = unicodedata.normalize('NFKC', self._legacy['full_text'])  # NFKD will change accents
        self.text = ' '.join(self.text.splitlines())  # Remove line breaks
        self.text = ' '.join(self.text.split())  # Remove repeated spaces

        self.mentions = self._legacy['entities'].get('user_mentions')
    ...
    def retweeted(self) -> bool:
        return self._legacy.get('retweeted')
